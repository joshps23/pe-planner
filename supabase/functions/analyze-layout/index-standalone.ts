...
- ALWAYS describe zones/areas using CONE references (e.g., "area between the red and blue cones")
- NEVER use percentages or coordinates in game instructions
- Use physical landmarks: "Start at the cone near the left side", "Move to the center cones"
- Reference cone colors or positions: "Pass through the gate formed by the two cones"
- Make it visual: "The four corner cones mark the treasure zones"
...
- Example GOOD: "Zone 1 is marked by the first two cones on the left"
- Example BAD: "Zone 1 is from 25% to 40% of the court"
    ...
    try {
      ...
      const response = await fetch(apiUrl, {
        ...
        body: JSON.stringify({
          ...
          generationConfig: {
            ...
            topK: 10,
            topP: 0.8,
            maxOutputTokens: 30000, // Increased for more comprehensive responses
            candidateCount: 1
          }
        })
      })
      ...
      if (data.candidates?.[0]?.content?.parts?.[0]?.text) {
        ...
        if (parsedResponse.layouts && Array.isArray(parsedResponse.layouts)) {
          parsedResponse.layouts.forEach((layout: any, layoutIndex: number) => {
            if (layout.elements && Array.isArray(layout.elements)) {
              ...
              layout.elements.forEach((el: any) => {
                if (el.type && elementCounts.hasOwnProperty(el.type)) {
                  elementCounts[el.type as keyof typeof elementCounts]++
                }
              })

              // Add missing cones if needed
              while (elementCounts.cone < equipmentCount.cones) {
                const x = 30 + (elementCounts.cone * 10) % 40
                const y = 30 + Math.floor(elementCounts.cone / 4) * 10
